  ...
  if(d == 0)
    {
	  ...
	  q*=0.78*std::exp(-pow(q/16.0,0.25));
      afo=std::min(15.0,2.171*std::log(1.0+4.77e-4*prop.hg[0]*prop.hg[1] *
	          prop.wn*q));
...
ascat( double d, prop_type &prop, propa_type &propa)
{
  ...
  if(d == 0.0)
  ...
  else
    {
      ...
      h0s=h0;
	  th=propa.tha+d*prop.gme;
	  ascatv=ahd(th*d)+4.343*std::log(47.7*prop.wn*pow(th,4.0))-0.1 *
	         (prop.ens-301.0)*std::exp(-th*d/40e3)+h0;
	}
  return ascatv;
}
  ...
  for(int j=0;j < 2;++j)
    {
	  if(kst[j] <= 0)
	  ...
	  else
	    {
		  ...
		  prop.he[j]=prop.hg[j]+(1.0+q)*std::exp(-std::min(20.0,2.0*prop.hg[j]/std::max(1e-3,prop.dh)));
	    }
	    ...
	    { 
		  ...
		  if(a5 < 1000.0)
		    { propa.ems=(a6-a5)/200e3;
		      propa.dx=std::max(propa.dlsa,std::max(propa.dla+0.3*xae *
			         std::log(47.7*prop.wn),(a5-propa.aed-propa.ems*d5) /
					 (propa.emd-propa.ems)));
...
double
curve(double const &c1, double const &c2, double const &x1,
  ...
  if(propv.lvar > 0)
    {   switch(propv.lvar)
	     { default:
		   ...
		   case 2:
		     dexa=std::sqrt(18e6*prop.he[0])+std::sqrt(18e6*prop.he[1]) +
...
void
z1sq1(double z[], const double &x1, const double &x2,
      double& z0, double& zn)
{
     ...
     double pctConf, double &dbloss, char *strmode, int &errnum)
{
  ...
  prop.hg[0] = tht_m;
  prop.hg[1] = rht_m;
  propv.klim = (int32_t) radio_climate;
  prop.ens = eno;
  prop.kwx = 0;
  ivar = (long) ModVar;
  ipol = (long) pol;
